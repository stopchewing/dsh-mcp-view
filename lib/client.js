window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function McpViewPanel(props) {
			const isOpen = react.useSyncExternalStore(openStore.subscribe, openStore.getSnapshot);
			// Note: `props.useSessions` (a slot hook) crashed with
			// "w is not a function" inside useSyncExternalStore, so the panel
			// deliberately does NOT call it — it always opens in the global view.
			// (Per-session scoping needs a live-verifiable source; revisit later.)
			if (!isOpen) return null;
			return react.createElement(McpViewPanelInner, { currentSessionId: "" });
		}
