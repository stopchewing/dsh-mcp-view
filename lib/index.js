 ...
 */
export function decompressSessionFile(path) {
...
export function __resetScanCacheForTests() {
    scanCache = null;
}
/** Scan the persisted session logs for `tool/call` events. */
export function scanLastUsed() {
    const now = Date.now();
    if (scanCache !== null && now - scanCache.at < SCAN_TTL_MS)
        return scanCache;
    const root = join(dshHome(), "sessions");
    const files = walkSessionFiles(root).slice(0, MAX_SCAN_FILES);
    const lastUsed = scanCache !== null ? new Map(scanCache.lastUsed) : new Map();
    const byTool = scanCache !== null ? new Map(scanCache.byTool) : new Map();
    const dayBuckets = scanCache !== null ? new Map(scanCache.dayBuckets) : new Map();
    ...
    for (const file of files) {
        ...
        seen.set(file, key);
        if (scanCache !== null && scanCache.files.get(file) === key)
            continue;
        if (budget <= 0) {
            capped = true;
            continue;
        ...
        for (const line of text.split("\n")) {
            ...
            if (event === null || event.type !== "tool/call" || event.data == null)
                continue;
            ...
            if (typeof event.time === "number") {
                ...
                if (prev === undefined || event.time > prev)
                    lastUsed.set(rawName, event.time);
            }
 ...
 */
export function parseMcpName(publicName, byName) {
...
export function resolveSessionScope(ctx, sessionId) {
    ...
    try {
        ...
        if (agent !== undefined && agent.scope !== undefined)
            return agent.scope;
        return undefined;
    }
    catch {
 ...
 */
export async function probeHealth(record) {
 ...
 */
export async function collectSnapshot(ctx, sessionId) {
    ...
    if (typeof sessionId === "string" && sessionId !== "") {
        ...
        try {
            ...
            if (session && typeof session.agentPreset === "string")
                serving.agentPreset = session.agentPreset;
        }
        catch (error) {
    ...
    try {
        for (const entry of ctx.loader.entries()) {
            ...
            if (options == null || options.name !== MCP_CLIENT_PACKAGE)
                continue;
    ...
    for (const schema of schemas) {
        ...
        if (parsed.server !== undefined) {
            ...
            if (typeof lastUsedMs === "number") {
                ...
                if (current === null || lastUsedMs > Date.parse(current))
                    parsed.server.lastUsedAt = tool.lastUsedAt;
            }
        }
        else {
